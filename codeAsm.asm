...
inicializa_vetores_loop:
	beq	$t5, $t0, inicializa_max		# verifica se t5 e t0 são iguais
    	...
    	addi	$t1, $t1, 4  		# Avança para o proximo elemento de a
    	addi	$t2, $t2, 4  		# Avança para o proximo elemento de b
    	addi	$t3, $t3, 4  		# Avança para o proximo elemento de c
    	addi	$t4, $t4, 4  		# Avança para o proximo elemento de d
...
max_value_loop:
	beq	$t5, $t0, inicializa_soma		# verifica se t5 e t0 são iguais
    	lw 	$t6, 0($t1)   		# Carrega c[i]
    	lw 	$t7, 0($t2)   		# Carrega d[i]
    	bgt	$t6, $t3, c_maior	# verifica se c[i] é maior que max
    	
verify_d:    	
    	bgt	$t7, $t3, d_maior	#verifica se d[i] é maior que max
    	
increment_max_value:
    	addi	$t5, $t5, 1  		# Incrementa i
    	addi	$t1, $t1, 4  		# Avança para o próximo elemento de c
    	addi	$t2, $t2, 4  		# Avança para o próximo elemento de d
    	j max_value_loop		
...
soma_loop:
	beq	$t5, $t0, inicializa_multi		# verifica se t5 e t0 são iguais
    	lw 	$t6, 0($t1)   		# Carrega c[i]
    	lw 	$t7, 0($t2)   		# Carrega d[i]
...
inicializa_multi:
	...
	blt	$t1, $0, max_negativo	# verifica se o meior valor encontrado é negativo

multi_loop:
	beq	$t5, $t1, verify_result	# verifica se i e maior valor são iguais
...
verify_result:
	bgt	$t3, $0, salva_sm	# verifica se o maior valor é negativo
...
troca_sinal_result:
	...
	#salva na memória
salva_sm:
	sw	$t9, ($t0)		# salva o resultado em sm
