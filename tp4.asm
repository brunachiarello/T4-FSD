...
#preenche os vetores c e d
inicializa_vetores_loop:
	beq	$t5, $t0, inicializa_max		# verifica se t5 e t0 sao iguais
...
# acha o maior valor entre os vetores C e D
inicializa_max:	
	xor	$t5, $t5, $t5			# zera i
	la	$t1, c				# c para o registrador t1
	la	$t2, d				# d para o registrador t2
	lw 	$t3, 0($t1)			# inicializa max value com c[0]	

max_value_loop:
	beq	$t5, $t0, inicializa_soma		# verifica se t5 e t0 sao iguais
...
increment_max_value:
    	addi	$t5, $t5, 1  			# Incrementa i
    	addi	$t1, $t1, 4  			# Avanca para o proximo elemento de c
    	addi	$t2, $t2, 4  			# Avanca para o proximo elemento de d
    	j max_value_loop		
...
#soma os vetores C e D
inicializa_soma:
	xor	$t5, $t5, $t5			# zera i
	la	$t1, c				# c para o registrador t1
	la	$t2, d				# d para o registrador t2
	xor	$t4, $t4, $t4			# inicializa um registrados para guardar a soma
	
soma_loop:
	beq	$t5, $t0, inicializa_multi		# verifica se t5 e t0 sao iguais
    	lw 	$t6, 0($t1)   			# Carrega c[i]
    	lw 	$t7, 0($t2)   			# Carrega d[i]

    	add	$t4, $t4, $t6			# soma t4 com a[i]
    	add	$t4, $t4, $t7			# soma t4 com b[i]

    	addi	$t5, $t5, 1  			# Incrementa i
    	addi	$t1, $t1, 4  			# Avan a para o proximo elemento de c
    	addi	$t2, $t2, 4  			# Avan a para o proximo elemento de d
    	j soma_loop
...
multi_loop:
	beq	$t5, $t1, verify_result		# verifica se t5 e t1 sao iguais
...
verify_result:
	bgt	$t3, $0, salva_sm			# verifica se o maior valor e negativo
